...
def fft2_em(real_space_wave, axes=(0, 1)):
    ...
    return cp.fft.ifft2(real_space_wave, axes=axes) * _normalization_for_axes(
        real_space_wave,
        axes,
    )
...
def ifft2_em(reciprocal_space_wave, axes=(0, 1)):
    ...
    return cp.fft.fft2(reciprocal_space_wave, axes=axes) / _normalization_for_axes(
...
def aberrations_from_parameters(params):
    ...
    a1_phase = cp.radians(params["A1_phase"])
    a2_phase = cp.radians(params["A2_phase"])
    b2_phase = cp.radians(params.get("B2_phase", 0))
    a3_phase = cp.radians(params["A3_phase"])
    s3_phase = cp.radians(params.get("S3_phase", 0))
...
def compute_probe_image(ctf_tensor, sigma=2.0):
    ...
    probe_wave = cp.fft.fft2(ctf_tensor, axes=(0, 1))  # EM convention: ifft is forward, fft is inverse.
    ...
    probe_sum = cp.sum(probe_image_final, axis=(0, 1), keepdims=True)
    probe_image_final = probe_image_final / cp.where(probe_sum == 0, 1, probe_sum)
    cp.get_default_memory_pool().free_all_blocks()
    gc.collect()

...
For one-coefficient-at-a-time calibration plots, run `notebooks/uno_coefficient_relationships.ipynb`. It sweeps C1, C3, A1, B2/C21, A2, A3, and S3/C32 separately, computes the corresponding Uno values from paired under/over-focus line profiles, and saves scalar, amplitude, and phase relationship plots, Fig. 8-style probe-shape galleries, and a CSV under `outputs/uno_relationships/`. For this diagnostic notebook, `C1_value` is the sign-flipped `-Cdf_value`, and `C3_value` is computed from the overfocused `Rho` term only; the original `Cdf_value` and `Cs_value` are still retained in the CSV.

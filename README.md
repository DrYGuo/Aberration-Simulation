...
For one-coefficient-at-a-time calibration plots, run `notebooks/uno_coefficient_relationships.ipynb`. It sweeps C1, C3, A1, B2/C21, A2, A3, and S3/C32 separately, computes the corresponding Uno values from paired under/over-focus line profiles, and saves scalar, amplitude, and phase relationship plots, Fig. 8-style probe-shape galleries, coupled C1/C3 scalar-response color maps, and a CSV under `outputs/uno_relationships/`. For this diagnostic notebook, `C1_value = -Cdf_value = -(1/N) sum_k (Xigma_under,k - Xigma_over,k)` and `C3_value = -mean(Rho_over)`. The original `Cdf_value` and paired-focus `Cs_value = (1/N) sum_k (Rho_under,k - Rho_over,k)` are still retained in the CSV.
